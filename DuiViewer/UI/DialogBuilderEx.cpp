...
CDialogBuilderEx::CDialogBuilderEx() : m_pCallback(NULL), m_pstrtype(NULL)
{

}

CControlUI* CDialogBuilderEx::Create(STRINGorID xml, LPCTSTR type, IDialogBuilderCallback* pCallback,
...
CControlUI* CDialogBuilderEx::Create(IDialogBuilderCallback* pCallback, CPaintManagerUI* pManager, CControlUI* pParent)
{
	m_pCallback = pCallback;
		...
		if (_tcsicmp(pstrClass, _T("Window")) == 0) {
			if (pManager->GetPaintWindow()) {
				...
				for (int i = 0; i < nAttributes; i++) {
					...
					if (_tcscmp(pstrName, _T("layered")) == 0 || _tcscmp(pstrName, _T("bktrans")) == 0) {
...
CMarkup* CDialogBuilderEx::GetMarkup()
{
	return &m_xml;
}

void CDialogBuilderEx::GetLastErrorMessage(LPTSTR pstrMessage, SIZE_T cchMax) const
{
	return m_xml.GetLastErrorMessage(pstrMessage, cchMax);
}

void CDialogBuilderEx::GetLastErrorLocation(LPTSTR pstrSource, SIZE_T cchMax) const
{
	return m_xml.GetLastErrorLocation(pstrSource, cchMax);
}

CControlUI* CDialogBuilderEx::_Parse(CMarkupNode* pRoot, CControlUI* pParent, CPaintManagerUI* pManager)
{
