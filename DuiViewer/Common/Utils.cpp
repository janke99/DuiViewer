...
void CUtils::DbgPrintfA(LPCSTR lpFmt, ...)
{
	char szBuf[1024];
	memset(szBuf, 0, 1024);
	va_list argList;
...
void CUtils::DbgPrintfW(LPCWSTR lpFmt, ...)
{
	WCHAR szBuf[1024];
	memset(szBuf, 0, 1024);
	va_list argList;
...
void CUtils::GetSpecialFolderDir(UINT uCode, std::wstring& chFolder)
{
	LPITEMIDLIST pidl;
...
std::string CUtils::Format(const char *fmt, ...)
{
...
std::wstring CUtils::Format(const wchar_t *fmt, ...)
{
...
int CUtils::RandomInt(UINT nMin, UINT nMax)
{
	srand((unsigned)time(NULL));
